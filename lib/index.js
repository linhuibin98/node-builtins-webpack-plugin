...
function polyfillPath(name) {
	return require.resolve(join("..", "lib", "polyfill", name));
}

const EMPTY_PATH = polyfillPath("empty");
...
function createAliasFilter({ excludeAliases, onlyAliases, additionalAliases }) {
	...
	if (additionalAliases.length > 0) {
		return (object) =>
			includeKeys(
				object,
				(key) =>
					(defaultPolyfills.has(key) && !excludeAliases.includes(key)) ||
					additionalAliases.includes(key)
			);
	}
	...
	return (object) =>
		includeKeys(
			object,
			(key) => defaultPolyfills.has(key) && !excludeAliases.includes(key)
		);
}
...
module.exports = class NodeBuiltinsPlugin {
	constructor(options = {}) {
		this.options = {
			excludeAliases: [],
			onlyAliases: [],
		...
		if (this.options.onlyAliases.length > 0) {
			if (
				...
				this.options.additionalAliases.length > 0
			) {
				throw new Error(
			...
			throw new Error(
				"excludeAliases and additionalAliases must not include the same items"
			);
		}
	}
	...
	apply(compiler) {
		...
		compiler.options.resolve.fallback = {
			...filter({
				...
				dns: EMPTY_PATH,
				dgram: EMPTY_PATH,
				child_process: polyfillPath("child_process"),
				cluster: EMPTY_PATH,
				module: polyfillPath("module"),
				net: EMPTY_PATH,
				readline: EMPTY_PATH,
				repl: EMPTY_PATH,
				tls: EMPTY_PATH,
